...
const CreateResource: React.FC = () => {
  ...
  const handleBackNavigation = () => {
    if (title.trim() || description.trim() || region.trim() || schedule.trim()) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <select value={category} onChange={e => setCategory(e.target.value as ResourceCategory)} className="w-full px-4 py-2 bg-white border border-[#EAE2D6] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#D4AF37]">
              {Object.values(ResourceCategory).map(t_val => <option key={t_val} value={t_val}>{t(`resourceCategories.${t_val}`)}</option>)}
            </select>
          </div>
          ...
          <div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} required rows={5} className="w-full px-4 py-2 bg-white border border-[#EAE2D6] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#D4AF37]" placeholder={t('createResource.form.descriptionPlaceholder')}></textarea>
          </div>
